...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        in_size: int = len(self.weights)
        result = []
        for j in range(len(self.bias)):
            y = minitorch.Scalar(0.0)
            for i in range(len(self.weights)):
               y += inputs[i] * self.weights[i][j].value + self.bias[j].value
            result.append(y)
        return result 
        # for i in range(in_size):
        #     y = minitorch.Scalar(0.0) 
        #     weight_i = self.weights[i]
        #     for j in range(len(weight_i)):
        #         y += inputs[i] * weight_i[j].value + self.bias[j].value
        #     result.append(y)
        # return result
