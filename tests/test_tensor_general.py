...
if numba.cuda.is_available():
    ...
    @pytest.mark.task3_3
    def test_sum_practice6() -> None:
        x = [1.0 for i in range(63)]
        b = minitorch.tensor(x)
        s = b.sum()[0]
        b2 = minitorch.tensor(x, backend=shared["cuda"])
        out = minitorch.sum_practice(b2)
    ...
    def test_mul_practice1() -> None:
        x1 = [[1 for i in range(2)] for j in range(2)]
        y1 = [[1 for i in range(2)] for j in range(2)]
...
@pytest.mark.task3_2
def test_bmm2() -> None:
    a = minitorch.rand((3, 2, 3), backend=FastTensorBackend)
    b = minitorch.rand((1, 3, 4), backend=FastTensorBackend)
    c = a @ b

    c2 = (a.view(3, 2, 3, 1) * b.view(1, 1, 3, 4)).sum(2).view(3, 2, 4)
...
def test_one_args_simple(
    data: DataObject
) -> None:
    "Run forward for all one arg functions above."
    t1 = data.draw(tensors(backend=shared['cuda']))
    name, base_fn, tensor_fn = 'dummy', MathTest.addConstant, MathTestVariable.addConstant
    t2 = tensor_fn(t1)
    print("=====output======" * 3)
    print(t2)
    for ind in t2._tensor.indices():
